...
- 如果需要新增测试，统一写在 `cbok/test/` 目录下（Python 包路径 `cbok.test`），不要继续分散到各 app 的 `tests.py`。
- `cbok/test/` 下按被测领域或模块分类存放，例如 CLI、alert、bbx、foundation、notification 等；测试文件名应能直接看出覆盖的功能范围。
