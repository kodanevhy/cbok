...
function finalize_startup_os_in_os() {
    ...
    startup_script="$4"
    filename=$(basename "$startup_script")
    remote_exec_via_jump "$address" kubectl exec -n openstack "$pod_name" -c "$container" -- cp $startup_script /opt/$filename
    remote_exec_via_jump "$address" kubectl exec -n openstack "$pod_name" -c "$container" -- sudo chmod 777 /opt/$filename
    remote_exec_via_jump "$address" kubectl exec -n openstack "$pod_name" -c "$container" -- sed -i '2d' /opt/$filename
}
