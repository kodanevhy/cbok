...
class GoogleAlertCrawler(base.BaseCrawler):
    ...
    def login(self, username, password):
        """Login to Google and return cookie"""

        return self.login_manager(username, password)

    def analysis_index(self):
        stashed_cookie = "with open and try to ping"
        if stashed_cookie:
            session_cookies = stashed_cookie
        else:
            self.login('kodanevhyz@gmail.com', 'Everlost584@')
            session_cookies = "with open"
